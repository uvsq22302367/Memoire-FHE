...
Double fonction : Contrairement au bootstrapping classique qui ne fait que "nettoyer" le bruit d'un chiffré, le bootstrapping programmable réduit le bruit tout en évaluant simultanément une fonction logique ou mathématique.

Utilisation de LUT (Look-Up Table) : Le processus utilise une table de correspondance. Pendant l'opération de réduction du bruit, le système applique la fonction encodée dans cette table à la donnée chiffrée.

Calcul non-linéaire : Il permet de calculer facilement des fonctions complexes ou non-linéaires (comme des comparaisons, des seuils ou des fonctions d'activation en IA) qui sont d'ordinaire très coûteuses en homomorphe.

Vitesse et efficacité : Grâce à la structure du Tore, cette opération s'effectue en quelques millisecondes, permettant d'enchaîner un nombre théoriquement infini d'opérations puisque le bruit est réinitialisé après chaque calcul.
\section{Conclusion}

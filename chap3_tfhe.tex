...
La sécurité du TFHE repose sur le problème TLWE, une adaptation du problème LWE classique au tore réel.
...
\]
où $\mathbf{a}$ est un vecteur aléatoire uniforme sur $\mathbb{T}^k$ et $e$ est un bruit gaussien de faible amplitude.
\end{definition}
...
TFHE permet d’évaluer des portes logiques (AND, OR, XOR) directement sur des échantillons TLWE. Comme toute fonction booléenne peut être décomposée en ces portes fondamentales, TFHE est capable d'exécuter n'importe quel circuit logique de manière chiffrée.
...
La grande innovation du TFHE réside dans son \textbf{bootstrapping}. Contrairement aux schémas précédents, il est \guillemotleft~programmable~\guillemotright : il peut évaluer une fonction arbitraire (via une \textit{Look-Up Table}) pendant le processus de réduction du bruit.

Cette opération est réalisée en quelques millisecondes, garantissant ainsi une profondeur de calcul théoriquement infinie.

\section{Conclusion}

Le schéma TFHE complète parfaitement le paysage de la cryptographie homomorphe. Si CKKS reste privilégié pour le calcul statistique approché, TFHE s'impose comme la solution de référence pour la logique booléenne exacte et le contrôle de flux chiffré.